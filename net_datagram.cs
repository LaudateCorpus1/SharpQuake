...
namespace SharpQuake
{
    internal class net_datagram : INetDriver
    {
        ...
        private void Stats_f()
        {
            if( QCommand.Argc == 1 )
            {
                QConsole.Print( "unreliable messages sent   = %i\n", net.UnreliableMessagesSent );
                QConsole.Print( "unreliable messages recv   = %i\n", net.UnreliableMessagesReceived );
                QConsole.Print( "reliable messages sent     = %i\n", net.MessagesSent );
                QConsole.Print( "reliable messages received = %i\n", net.MessagesReceived );
                QConsole.Print( "packetsSent                = %i\n", packetsSent );
                QConsole.Print( "packetsReSent              = %i\n", packetsReSent );
                QConsole.Print( "packetsReceived            = %i\n", packetsReceived );
                QConsole.Print( "receivedDuplicateCount     = %i\n", receivedDuplicateCount );
                QConsole.Print( "shortPacketCount           = %i\n", shortPacketCount );
                QConsole.Print( "droppedDatagrams           = %i\n", droppedDatagrams );
            }
            else if( QCommand.Argv( 1 ) == "*" )
            {
            ...
            {
                qsocket_t sock = null;
                string cmdAddr = QCommand.Argv( 1 );

                foreach( qsocket_t s in net.ActiveSockets )
                    if( QCommon.SameText( s.address, cmdAddr ) )
                    {
                        sock = s;
                        break;
                    }
        ...
        {
            QConsole.Print( "canSend = {0:4}   \n", s.canSend );
            QConsole.Print( "sendSeq = {0:4}   ", s.sendSequence );
            QConsole.Print( "recvSeq = {0:4}   \n", s.receiveSequence );
            QConsole.Print( "\n" );
        }
        ...
        public void Init()
        {
            ...
            QCommand.Add( "net_stats", Stats_f );

            if( QCommon.HasParam( "-nolan" ) )
                return;
        ...
        public qsocket_t InternalCheckNewConnections()
        {
            ...
            net.Reader.Reset();
            int control = QCommon.BigLong( net.Reader.ReadLong() );
            if( control == -1 )
                return null;
            ...
            if( command == CCReq.CCREQ_SERVER_INFO )
            {
                ...
                QCommon.WriteInt( net.Message.Data, 0, QCommon.BigLong( NetFlags.NETFLAG_CTL |
            ...
            if( command == CCReq.CCREQ_RULE_INFO )
            {
                ...
                // search for the next server QCVar
                while( var != null )
                {
                    if( var.IsServer )
                        break;
        ...
        public int GetMessage( qsocket_t sock )
        {
            ...
            while( true )
            {
                ...
                {
#if DEBUG
                    QConsole.DPrint("Forged packet received\n");
                    QConsole.DPrint("Expected: {0}\n", StrAddr(sock.addr));
                    QConsole.DPrint("Received: {0}\n", StrAddr(readaddr));
#endif
                    continue;
                }
                ...
                length = QCommon.BigLong( header.length );
                ...
                uint sequence = (uint)QCommon.BigLong( header.sequence );
                packetsReceived++;
                ...
                if( ( flags & NetFlags.NETFLAG_UNRELIABLE ) != 0 )
                {
                    ...
                    {
                        QConsole.DPrint( "Got a stale datagram\n" );
                        ret = 0;
                        break;
                    }
                    if( sequence != sock.unreliableReceiveSequence )
                    {
                        ...
                        QConsole.DPrint( "Dropped {0} datagram(s)\n", count );
                    }
                ...
                if( ( flags & NetFlags.NETFLAG_ACK ) != 0 )
                {
                    ...
                    {
                        QConsole.DPrint( "Stale ACK received\n" );
                        continue;
                    }
                    if( sequence == sock.ackSequence )
                    {
                        ...
                        if( sock.ackSequence != sock.sendSequence )
                            QConsole.DPrint( "ack sequencing error\n" );
                    }
                    else
                    {
                        QConsole.DPrint( "Duplicate ACK received\n" );
                        continue;
                    }
                ...
                {
                    header.length = QCommon.BigLong( net.NET_HEADERSIZE | NetFlags.NETFLAG_ACK );
                    header.sequence = QCommon.BigLong( (int)sequence );
        ...
        public int SendMessage( qsocket_t sock, QMessageWriter data )
        {
            ...
            PacketHeader header;
            header.length = QCommon.BigLong( packetLen | NetFlags.NETFLAG_DATA | eom );
            header.sequence = QCommon.BigLong( (int)sock.sendSequence++ );
            sys.StructureToBytes( ref header, _PacketBuffer, 0 );
        ...
        public int SendUnreliableMessage( qsocket_t sock, QMessageWriter data )
        {
            ...
            PacketHeader header;
            header.length = QCommon.BigLong( packetLen | NetFlags.NETFLAG_UNRELIABLE );
            header.sequence = QCommon.BigLong( (int)sock.unreliableSendSequence++ );
            sys.StructureToBytes( ref header, _PacketBuffer, 0 );
        ...
        private void InternalSearchForHosts( bool xmit )
        {
            ...
            while( true )
            {
                ...
                net.Reader.Reset();
                int control = QCommon.BigLong( net.Reader.ReadLong() );// BigLong(*((int *)net_message.data));
                //MSG_ReadLong();
                if( control == -1 )
                    continue;
                if( ( control & ( ~NetFlags.NETFLAG_LENGTH_MASK ) ) != NetFlags.NETFLAG_CTL )
        ...
        {
            // see if we can resolve the QHost name
            ...
            // connect to the QHost
            if( net.LanDriver.Connect( newsock, sendaddr ) == -1 )
                goto ErrorReturn;

            // send the connection request
            QConsole.Print( "Connecting to " + sendaddr + "\n" );
            QConsole.Print( "trying...\n" );
            Scr.UpdateScreen();
            ...
            for( int reps = 0; reps < 3; reps++ )
            {
                ...
                do
                {
                    ...
                    if( ret > 0 )
                    {
                        ...
                        {
#if DEBUG
                            QConsole.Print("wrong reply address\n");
                            QConsole.Print("Expected: {0}\n", StrAddr(sendaddr));
                            QConsole.Print("Received: {0}\n", StrAddr(readaddr));
                            Scr.UpdateScreen();
#endif
                            ret = 0;
                            continue;
            ...
            QConsole.Print( "Connection accepted\n" );
...
ErrorReturn2:
        ...
        private int ReSendMessage( qsocket_t sock )
        {
            ...
            PacketHeader header;
            header.length = QCommon.BigLong( packetLen | ( NetFlags.NETFLAG_DATA | eom ) );
            header.sequence = QCommon.BigLong( (int)( sock.sendSequence - 1 ) );
            sys.StructureToBytes( ref header, _PacketBuffer, 0 );
            Buffer.BlockCopy( sock.sendMessage, 0, _PacketBuffer, PacketHeader.SizeInBytes, dataLen );
